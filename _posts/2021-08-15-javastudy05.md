...
클래스에 어떠한 생성자도 선언하지 않은 경우, 컴파일러가 자동으로 `디폴트 생성자`를 만들어 준다. 어떠한 생성자 하나라도 선언했다면 컴파일러는 디폴트 생성자를 만들지 않는다.  
...
빌더 패턴을 사용하기 위해서는 클래스 내에 static inner class로 Builder를 생성하고, 그 안에서 `return this;`를 통해서 메소드 체이닝을 걸어 매개변수를 받을 수 있도록 한다. 그리고 마지막으로 `.build()`를 호출함으로써 객체를 반환한다. 
...
# 오늘은 여기까지, 내일 지금까지 쓴 거 검수하고 Reflection 부분 추가하고 과제 추가하면 끝 
