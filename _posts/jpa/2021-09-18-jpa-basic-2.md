...
임베디드 타입을 사용하기 위해서는 값 타이블 정의하는 클래스에 `@Embeddable`, 값 타입을 사용하는 곳에 `@Embedded`를 사용하면 된다. 이 때 기본 생성자가 필수이고, 두 개의 어노테이션 중에 하나만 명시해도 컴파일에 문제는 없으나 명확성을 위해 둘 다 명시할 것이 좋다.
