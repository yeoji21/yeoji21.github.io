...
컬렉터는 `Collector인터페이스`를 구현한 것으로, 직접 구현하거나 미리 작성된 것을 사용하면 된다. `Collectors클래스`는 미리 작성된 다양한 종류의 컬렉터를 반환하는 `static method`를 가지고 있다. 
...
`groupingBy()`로 그룹화를 하면 기본적으로 `Map`의 value를 `List<T>`에 담는다. 그래서 위의 문장은 `toList()`가 생략된 것과 같다.
```java
Map<Integer, List<Student>> studentByBan = Stream.of(studentArr)
