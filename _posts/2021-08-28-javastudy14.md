...
그리고 위의 `sort()`의 정의를 보면 `static` 메소드인 것을 알 수 있는데, 앞서 설명한 것처럼 `static` 멤버에는 타입 매개변수를 사용할 수 없지만, 메소드에 제네릭 타입을 선언하고 사 용하는 것은 가능하다.  
...
그렇다면 이번에는 타입 파라미터에 상항을 두고 결과를 확인해보자.
```java
...
예제를 위한 `Monster` 클래스와 이 것을 상속받는 `Digimon` 클래스
```java
...
- 배열은 공변(convariant)이다.  
    ...
    예를 들어, `String`은 `Object`의 하위타입이기 때문에 `String[]`은 O`bject[]`의 하위타입이다.
...
- 제네릭 타입은 정보가 런타임에는 `소거(erasure)`된다.  
    원소 타입을 컴파일타임에만 검사하며 런타임에는 알수조차 없다는 뜻이다.  
    > 무조건은 아니고 경우에 따라 알 수 있는 방법이 있긴한데, 포스트의 뒷 부분에 언급한다. 
