...
이 때 주의해야할 점은 위의 catch 블럭에서 걸리면 아래 catch 블럭은 호출되지 않는다. 예를 들어 위에 블럭에 `RuntimeException`을 잡아놓고, 그 밑에 `IllegalArgumentException`을 잡아놨다면 잘못된 코드이다. 이런 경우가 발생하면 IDE에서 에러로 알려준다. 
...
```
이렇게 메소드의 선언부에 예외를 선언함으로써 해당 메소드에서 어떠한 예외가 처리되어야 하는지 직관적으로 파악할 수 잇따.  

하지만 메소드에 예외를 선언하는 것은 예외를 처리하는 것이 아니고, 해당 메소드를 호출한 메소드에게 예외를 던져서 예외처리를 떠넘기는 것이다.  
...
그렇기 때문에 finally 블럭 내에서 return문을 사용하는 것은 안티패턴이므로 사용하지 않아야 한다. 만약 try문과 catch문에서 return을 호출하고 싶어도 항상 finally의 return이 호출되기 때문에 의도치 않은 흐름으로 프로그램이 동작할 수 있기 때문이다.  
