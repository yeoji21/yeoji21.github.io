...
위의 예제코드 중에도 나왔고 예외 처리 시 많이 사용하는 메소드인 `printStackTrace()`와 `getMessage()`가 바로 Throwable 클래스의 메소드이다. 
...
1주차에서 배운 JVM의 구조를 떠올려보면, 메소드 실행 시 메모리의 `Stack` 영역에 `Stack Frame`이 쌓이게 된다. 즉, Stack 영역은 Stack Frame을 저장하는 영역이며, JVM은 오직 Stack Frame을 push하고 pop하는 작업을 한다.  
